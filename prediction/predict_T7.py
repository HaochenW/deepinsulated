...
if __name__ == '__main__':
    sequence,LB_exp,M9_exp,control,record_dict,record_average,file  = get_data_T7()
    ...
    for i in range(20):
        ...
        for j in range(iter_num):
            train_feature, train_label, test_feature, test_label = train_test_split(sequence_onehot, LB_exp, split_th = (i+75)/100)
            ...
            model.fit(train_feature,train_label, epochs = 1000, batch_size = 8, validation_split=0.1, \
                      callbacks=[EarlyStopping(patience=10),ModelCheckpoint(filepath='weight_CNN_LB_induced.h5',save_best_only=True)],shuffle=True) 
            model.load_weights('weight_CNN_LB_induced.h5')
    ...
    plt.legend()
    plt.title('relationship LB_exp_average 15_experiments_induced')
    plt.savefig('relationship_record_LB_exp_average_induced.jpg')
    ...
    plt.ylim(y_lim)
    plt.xlim(x_lim)
    plt.title('scatter plot for the last iteration LB induced')
    plt.savefig('scatter plot_LB_exp_induced.jpg')
